...
const uri = process.env.MONGODB_URI || 'mongodb://127.0.0.1:27017/chatsapp';
...
app.use(cors({ origin: corsOrigin, methods: ['GET', 'POST', 'DELETE', 'PUT'], allowedHeaders: ['Authorization', 'Content-Type'] }));
...
const formatTimestamp = (date) => {
  return date.toLocaleString('en-US', {
    ...
    second: '2-digit',
    hour12: false
  }).replace(/,/, ''); // e.g., "05/07/2025 14:30:45"
};
...
app.get('/admin/messages/:userId', verifyToken, verifyAdmin, async (req, res) => {
  try {
    ...
    const formattedMessages = messages.map(msg => ({
      ...msg,
      formattedTimestamp: msg.formattedTimestamp || formatTimestamp(msg.timestamp)
    }));
...
app.post('/add-contact', verifyToken, async (req, res) => {
  try {
    ...
    if (!contactUsername?.trim()) return res.status(400).json({ message: 'Username is required' });
    if (currentUser.username === contactUsername) return res.status(400).json({ message: 'Cannot add yourself' });
    ...
    res.json({ message: 'Contact added successfully', contact: contact.username });
  } catch (error) {
